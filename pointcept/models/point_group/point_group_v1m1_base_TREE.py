...
import numpy as np # added to get the instance labels
...
@MODELS.register_module("TREE-PG-v1m1")
class TREE_PointGroup(nn.Module):
    def __init__(
        self,
        backbone,
    ...
    def forward(self, data_dict):
        ...
        # origin_coord = data_dict["origin_coord"] # Added: to get the coordinates
        ...
        if not self.training:
            ...
            # orig_coord = coord.detach().clone() # Added: to get the coordinates # WRONG, this is the grid sampled coordinates
            # orig_coord = origin_coord.detach().clone() # Added: to get the coordinates
            shifted_coord = center_pred.detach().clone() # Added: to get the shifted coordinates
            center_pred /= self.voxel_size
            logit_pred = F.softmax(logit_pred, dim=-1)
            semantic_pred = torch.max(logit_pred,1)[1]  # [n] # Added: The original semantic prediction
            segment_pred = torch.max(logit_pred, 1)[1]  # [n]
            idx = segment_pred != 0 # Added: Remapping class
            segment_pred[idx] = 1 # Added: Remapping class
            # cluster
            mask = (
                ~torch.concat(
                    [
            ...
            # Added: to get the instance labels
            pred_label = np.transpose(pred_masks)
            if pred_label.sum() > 0:
            ...
            return_dict["pred_label"] = pred_label # added to get the instance labels
            return_dict["pred_classes"] = pred_classes
            return_dict["pred_semantic"] = semantic_pred # Added: to get the original output of semantic classes
            # return_dict["orig_coord"] = orig_coord # Added: to get the original coordinates
            return_dict["shifted_coord"] = shifted_coord # Added: to get the shifted coordinates
        return return_dict
